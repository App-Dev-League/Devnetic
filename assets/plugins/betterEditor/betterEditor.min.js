...
interact('.vertical-divider')
  .resizable({
    ...
    listeners: {
      move (event) {
        var target = event.target.parentElement.children[0].querySelector(".project-module-tabs")
        var right = event.target.parentElement.children[2]

        var x = (parseFloat(event.target.getAttribute('data-x')) || 0)
        var y = (parseFloat(event.target.getAttribute('data-y')) || 0)

        event.rect.width -= 100

        target.style.width = "calc(" + event.rect.width + "px + "+window.innerWidth/2+"px)"
        right.style.width = "calc(55% - " + (event.rect.width+100) + "px)"
        right.style.transform = "translateX(" + event.rect.width + "px)"
        // translate when resizing from top or left edges
        x += event.rect.width
        y += event.deltaRect.top
        
        event.target.style.transform = "translateX(" + event.rect.width + "px)"
