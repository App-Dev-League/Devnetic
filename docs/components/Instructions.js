		...
		<br>
		 ...
		 - .cpp
			...
			${(this.state.elements||[]).map(t=>{if("code"==t.type)return`
							...
							`;if("image"==t.type){let e=t,i=(e.width||(e.width="90%"),"");return Object.entries(e).forEach(([e,t])=>{"src"!==e&&"type"!==e&&(i+=e+`: ${t};`)}),`<div class="image-wrapper info-text"><img src="${t.src}" style="display: block; margin-left: auto; margin-right: auto; ${i}"></div>`}if("iframe"!=t.type)return`<pre class="info-text">${codeTemplateToCode(t.content||"")}</pre>`;{let e=t,i=(e.width||(e.width="90%"),"");return Object.entries(e).forEach(([e,t])=>{"src"!==e&&"type"!==e&&(i+=e+`: ${t};`)}),`<div class="image-wrapper info-text"><iframe src="${t.src}" style="display: block; margin-left: auto; margin-right: auto; ${i}" onload="resizeIframe(this)"></iframe></div>`}}).join("")}
			...
			<div class="hints">
				...
				${(this.state.hints||[{elements:[{type:"text",content:""}]}]).map(e=>(window.maxHints=this.state.hints.length-1,`<div class="hint none">
						${e.elements.map(t=>{if("code"==t.type)return`
										...
										`;if("image"!=t.type)return`<pre class="info-text">${codeTemplateToCode(t.content||"")}</pre>`;{let e=t,i=(e.width||(e.width="90%"),"");return Object.entries(e).forEach(([e,t])=>{"src"!==e&&"type"!==e&&(i+=e+`: ${t};`)}),`<div class="image-wrapper info-text"><img src="${t.src}" style="display: block; margin-left: auto; margin-right: auto; ${i}"></div>`}}).join("")}
					</div>`)).join("")}
